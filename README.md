...
2. Run the `active-stations.py` script locally using the INFLUX_USERNAME and INFLUX_PASSWORD credentials for that server (see CHORDS_ADMIN_PW). Executable via CLI: `python3 active-stations.py <path/to/download/files> <chords portal name>` or via library import.

## Data Gap Filler
The `data_gap_filler.py` script fills in time series data gaps for a directory of csv's downloaded from a CHORDS portal.
Executable via CLI: `python3 data_gap_filler.py <data_directory> <sampling_rate> <data_destination>` or via library import.
